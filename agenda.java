...
{   
   ...
   public static void main(String args[])
   {
	...
	// ------------------------------------
	// Grabar los datos al archivo de texto
	// ------------------------------------
	static void grabarAgenda()
	{
		// Variable de indice
		int indice;
		...
		{			
			...
			FileWriter xFile = new FileWriter(NOM_ARCHIVO,false);
						
			// Creamos el objeto para el Buffer de Escritura
			BufferedWriter xEscritor = new BufferedWriter(xFile);
			
			// Mensaje
			System.out.println("Grabando la Agenda ...");

			// Ciclo para grabar
			for (indice=0; indice<registros; indice++)
			{
				// Verifica que esté activo
				if (matrizAgenda[indice][COL_STATUS].equals(MARCA_ACTIVO))
				{
					// Incrementa el contador
					contador++;

					// Actualiza el Id
					matrizAgenda[indice][COL_ID] = String.valueOf(contador);
					
					// Graba cada dato separado por coma
					xEscritor.write(matrizAgenda[indice][COL_ID]);
					xEscritor.write(",");
					xEscritor.write(matrizAgenda[indice][COL_NOM]);
					xEscritor.write(",");
					xEscritor.write(matrizAgenda[indice][COL_APE]);
					xEscritor.write(",");
					xEscritor.write(matrizAgenda[indice][COL_TEL_CASA]);
					xEscritor.write(",");
					xEscritor.write(matrizAgenda[indice][COL_TEL_MOVIL]);
					xEscritor.write(",");
					xEscritor.write(matrizAgenda[indice][COL_FEC_NAC]);

					// Graba el cambio de linea
					xEscritor.newLine();
			...
			System.out.println("Se ha finalizado de grabar la Agenda con "+contador+" registros");
	...
	static void agregarContacto() 
    {
		...
		matrizAgenda[registros][COL_ID] = String.valueOf(registros+1);
	...
	static void actualizarContacto() 
    {
        ...
        else 
        {
			...
			// Verifica que esté activo
			if (matrizAgenda[registroActualizar][COL_STATUS].equals(MARCA_ACTIVO))
			{
				// Solicita los datos
				System.out.println("Captura el Nombre, Apellido, teléfono de casa, teléfono móvil y fecha de nacimiento");
				System.out.println("Dando <Enter> despues de cada dato");
				...
				// Lee los datos
				teclado.nextLine();  // Libero buffer
				nombre   = teclado.nextLine();
				apellido = teclado.nextLine();
				telCasa  = teclado.nextLine();
				telMovil = teclado.nextLine();
			...
			{
				// Mensaje
				System.out.println("El Registro se encuentra borrado ...");				
			}
	...
	static void eliminarContacto() 
    {
        ...
        else 
        {
			...
			if (matrizAgenda[registroEliminar-1][COL_STATUS].equals(MARCA_BORRADO)) 
            {
				// Mensaje
